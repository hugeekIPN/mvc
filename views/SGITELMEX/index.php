<?php include_once('header.php');?>
	<div class="container btn-opciones">
		<ul class="nav nav-bar dashboard col-md-8">
			<li>
				<div class="form-group form">
		   			<div class="col-xs-2 uno">
		   				<img src="iconos/Recurso 1.png" alt="Buscar" class="imgs-dashboard">
			      		<p>Consulta</p>
		    		</div>
		    		
		  		</div>
		  		<ul class="col-xs-2 nav nav-bar consulta">
		    			<li><a href="">Especies</a></li>
		    			<li><a href="#">Eventos</a></li>
		    			<li><a href="#">Proveedores/Donatarios</a></li>
		    			<li><a href="#">Usuarios</a></li>
		    			<li><a href="#">Cuentas</a></li>
		    			<li><a href="#">Años</a></li>
		    			<li><a href="#">Programas</a></li>
		    		</ul>
	  		</li>
	  		<li>
		  		<div class="form-group">
		   			<div class="col-xs-2 dos">
		   				<img src="iconos/Recurso 2.png" alt="Buscar" class="imgs-dashboard">
			      		<p>Captura</p>
		    		</div>
		  		</div>
		  		<ul class="col-xs-2 nav nav-bar captura">
		    			<li><a href="#">Apoyos</a></li>
		    			<li><a href="#">Gastos</a></li>
		    			<li><a href="#">Programas</a></li>
		    			
		    		</ul>
	  		</li>
	  		<li>
		  		<div class="form-group">
		   			<div class="col-xs-2 tres">
			      		<img src="iconos/Recurso 3.png" alt="Buscar" class="imgs-dashboard">
			      		<p>Reportes</p>
		    		</div>
		  		</div>
		  		<ul class="col-xs-2 nav nav-bar reportes">
		    			<li><a href="#">Proveedores/Donatarios</a></li>
		    			<li><a href="#">Eventos</a></li>
		    			<li><a href="#">Recurrente Provedor</a></li>
		    			<li><a href="#">Recurrente Donatario</a></li>
		    			<li><a href="#">Captura Diaria</a></li>
		    			<li><a href="#">Apoyo por Donatario</a></li>
		    			<li><a href="#">Apoyo por Proveedor</a></li>
		    			<li><a href="#">Apoyo por Evento</a></li>
		    			<li><a href="#">Evento por Programa</a></li>
		    		</ul>
	  		</li>
	  		<li class="buttons ">
		  		<div class="form-group ">
		   			<div class="col-sm-3 as">
			      		<button type="submit" class=" btn-md-2 ayuda">Ayuda</button>
			      		<button type="submit" class=" btn-md-2 salir">Salir</button>
		    		</div>	
	  			</div>
  			</li>
		</ul>
	</div>	
	<footer>
		<p><strong>Copyright Fundación Telmex 2017</strong></p>
		<p><small>Todos los Derechos Reservados</small></p>

	</footer>
</body>
<!--<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>
 	<script> 
	$(document).ready(function(){
	    $("div.uno").hover(function(){
	        $("div.uno").animate({
	            height: '80px'
	        });
	    });
	     $("div.dos").hover(function(){
	        $("div.dos").animate({
	            height: '80px'
	        });
	    });
	      $("div.tres").hover(function(){
	        $("div.tres").animate({
	            height: '80px'
	        });
	    });
	});
	</script>-->
</html>
